...
function SplitGroup(props, ref) {
    ...
    var recordRef = (0, react_1.useRef)(new Array(panes.length).fill({
        moving: false,
        startOffset: 0,
        startPosition: 0,
    }));
    var paneContainers = (0, react_1.useRef)([]);
    var movingIndex = (0, react_1.useRef)(0);
    var prevOffsets = (0, react_1.useRef)([]);
    var _f = __read((0, react_1.useState)(new Array(panes.length).fill(defaultOffset)), 2), offsets = _f[0], setOffsets = _f[1];
    var _g = __read((0, react_1.useState)(false), 2), isMoving = _g[0], setIsMoving = _g[1];
    var _h = __read((0, react_1.useState)(new Array(panes.length).fill(0)), 2), triggerSize = _h[0], setTriggerSize = _h[1];
    var _j = __read((0, react_1.useState)(new Array(Math.max(panes.length - 1, 0)).fill({ prev: false, next: false })), 2), collapsedStatus = _j[0], setCollapsedStatus = _j[1];
    ...
    var classNames = (0, classNames_1.default)(prefixCls, prefixCls + "-" + (isHorizontal ? DIRECTION_HORIZONTAL : DIRECTION_VERTICAL), (_a = {}, _a[prefixCls + "-moving"] = isMoving, _a), (_b = {}, _b[prefixCls + "-rtl"] = rtl, _b), className);
    var Tag = component;
    ...
    function formatSize(size) {
        ...
        if (!size || ((0, is_1.isNumber)(size) && size < 0)) {
            return 0;
        }
        var percent = (0, is_1.isString)(size) ? parseFloat(size) / totalPX : size;
    ...
    var getCollapsedConfig = function (index) {
        ...
        if (!(0, is_1.isObject)(collapsible)) {
    ...
    return (react_1.default.createElement(Tag, __assign({}, (0, omit_1.default)(rest, ['onMovingStart', 'onPaneResize', 'onMoving', 'onMovingEnd']), { style: style, className: classNames, ref: wrapperRef }), panes.map(function (pane, index) {
        ...
        var prevConfig = (0, is_1.isObject)(collapsible) && (0, is_1.isObject)(collapsible.prev) ? collapsible.prev : {};
        var nextConfig = (0, is_1.isObject)(collapsible) && (0, is_1.isObject)(collapsible.next) ? collapsible.next : {};
        return (react_1.default.createElement(react_1.default.Fragment, { key: index },
            react_1.default.createElement("div", { className: prefixCls + "-pane", style: { flexBasis: getPaneSize(index) }, ref: function (el) { return (paneContainers.current[index] = el); } }, content),
            !disabled && index !== panes.length - 1 && (react_1.default.createElement(resize_trigger_1.default, { className: prefixCls + "-trigger", direction: isTriggerHorizontal ? DIRECTION_HORIZONTAL : DIRECTION_VERTICAL, icon: icon, onResize: function (e) { return onTriggerResize(e, index); }, onMouseDown: function (e) { return onTriggerMouseDown(e, index); }, collapsible: {
                    prev: hasPrev
                        ? {
...
var SplitGroupComponent = (0, react_1.forwardRef)(SplitGroup);
SplitGroupComponent.displayName = 'ResizeBoxSplitGroup';
exports.default = SplitGroupComponent;
